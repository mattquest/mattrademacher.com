<template>
  <v-app id="vApp" style="background: none">
    ...
    <!--SNACKBAR-->
    <v-snackbar v-model="snackBar.show" color="white" class="blue-grey--text" bottom>
      <v-flex class="text-center font-weight-bold">
        {{ snackBar.text }}
      </v-flex>
    </v-snackbar>
    <v-footer absolute class="font-weight-medium grey--text" dark>
      <v-col class="text-center" cols="12">
        {{ new Date().getFullYear() }} — <strong>Wheelmaker</strong>
      </v-col>
    </v-footer>
  </v-app>
</template>
